...
def fetchPly(path, axis_alignment=None):
    ...
    if axis_alignment is not None:
        ...
        positions = (positions_h @ axis_alignment.T)[:, :3]    # (N,3)
...
def load_axis_alignment(txt_path):
    with open(txt_path, "r") as f:
        first_line = f.readline().strip()  # 只读第一行
...
def readCamerasFromTransforms(path, transformsfile, depths_folder, white_background, is_test, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_name = os.path.normpath(os.path.join(path, frame["file_path"]+extension))

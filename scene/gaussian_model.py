...
class GaussianModel:
    ...
    def capture_instance_feature(self):
        return (
            self.active_sh_degree,
            self._xyz,
            self._features_dc,
    ...
    def capture_semantic_feature(self):
        return (
            self.active_sh_degree,
            self._xyz,
            self._features_dc,
            ...
            self._opacity,
            self._cluster_indices,
            self._cluster_language_feature, 
            self.max_radii2D,
            self.xyz_gradient_accum,
            self.denom,
            self.optimizer.state_dict(),
    ...
    def restore_language_features(self, model_args, training_args):
        ...
        self.denom = denom
        self.optimizer.load_state_dict(opt_dict)
    def restore_instance_features(self, model_args, training_args):
        (self.active_sh_degree, 
        self._xyz, 
        self._features_dc, 
        self._features_rest,
        ...
        self.denom = denom
        self.optimizer.load_state_dict(opt_dict)
    def restore_semantic_features(self, model_args, training_args):
        (self.active_sh_degree, 
        self._xyz, 
        self._features_dc, 
        self._features_rest,
    ...
    def accumulate_gaussian_feature_per_view(self, gt_language_feature, gt_mask, mask, significance=None, means2D=None, feature_dim=512):
        ...
        self._language_feature_weight[mask] += significance.unsqueeze(-1) * gt_batch_mask
        self._language_feature[mask] += significance.unsqueeze(-1) * gt_batch_features * gt_batch_mask

        self._activated_views[mask] = torch.where(
            gt_batch_mask.squeeze(-1),
            1,
    ...
    def find_root(self, x, mapping):
        while x in mapping and mapping[x] != x:
            x = mapping[x]
        return x

    def backproject_2d(self, args, x, y, depth, intrinsics, pose):
        ...
        x = ((x - cx) * z) / fx 
        y = ((y - cy) * z) / fy
        camera_points = torch.stack([x, y, z, torch.ones_like(z)], dim=1).to(torch.float64).to(args.device)  # (N, 4)
        # 相机坐标系 -> 世界坐标系
        world_points = (pose.float() @ camera_points.float().T).T[:, :3]  # (N, 3)
    ...
    def assign_cluster_indices_dep(self, args, seg_map, depth_image, mask, means2D, proj_depths, iou_thresh=0.2):

        if not hasattr(self, "_cluster_indices"):
            self._cluster_indices = nn.Parameter(
                torch.zeros((self._xyz.shape[0],), dtype=torch.long, device=args.device),
                requires_grad=False
            )
        batch_y = torch.clamp(means2D[:, 1], min=0, max=seg_map.shape[0] - 1)
        batch_x = torch.clamp(means2D[:, 0], min=0, max=seg_map.shape[1] - 1)
      
        gt_depth = depth_image[batch_y.long(), batch_x.long()].float() / 1000  
      
        valid_gs = torch.isclose(gt_depth, proj_depths, rtol=0.05)
        gt_seg_map = seg_map[batch_y.long(), batch_x.long()][valid_gs] 
        global_mask_indices = mask.nonzero(as_tuple=True)[0][valid_gs]  

        valid_seg_mask = gt_seg_map >= 0
        gt_seg_map = gt_seg_map[valid_seg_mask]
        global_mask_indices = global_mask_indices[valid_seg_mask.squeeze(-1)]             
        
        unique_labels, label_indices = torch.unique(gt_seg_map, return_inverse=True)
        num_labels = unique_labels.shape[0]
        current_cluster_labels = self._cluster_indices[global_mask_indices] # (N_visible,)
   
        cluster_ids_per_label = torch.full((num_labels,), 0, dtype=torch.long, device=args.device)

        for i in range(num_labels):
      
            label_mask = label_indices == i
            cur_indices = label_mask.nonzero(as_tuple=True)[0]
            if cur_indices.numel() == 0:
                continue
            
            label_cluster_ids = current_cluster_labels[label_mask.squeeze(-1)]
            label_cluster_ids = label_cluster_ids[label_cluster_ids > 0]

            if label_cluster_ids.numel() > 0 :
                mode_cluster = torch.bincount(label_cluster_ids).argmax()

                if len(label_cluster_ids[label_cluster_ids == mode_cluster]) / cur_indices.numel() > iou_thresh:
                    cluster_ids_per_label[i] = mode_cluster
                else: 
                    cluster_ids_per_label[i] = 0
               
            else:
                cluster_ids_per_label[i] = 0 

        new_mask = cluster_ids_per_label == 0
        new_cluster_ids = torch.arange(
            int(self._cluster_indices.max().item()) + 1,
            int(self._cluster_indices.max().item()) + 1 + new_mask.sum().item(),
            device=args.device
        )
        cluster_ids_per_label[new_mask] = new_cluster_ids

        final_cluster_ids = cluster_ids_per_label[label_indices]         
        unassigned = current_cluster_labels == 0
        self._cluster_indices[global_mask_indices[unassigned]] = final_cluster_ids[unassigned].squeeze(-1)
